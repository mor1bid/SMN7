...
for (int i = 0, bar = 1; bar<=bnum; bar++)
{
        for (int col = 1; col<=cnum; i++, col++)
        {
            ...
            else if (mycol>cnum || mycol<=0 || mybar>bnum || mybar<=0) { Console.WriteLine("Такого элемента нет."); break; }
            //else if (mybar>bnum || mybar<=0) { Console.WriteLine("Такого элемента нет."); return; }
        }
}
